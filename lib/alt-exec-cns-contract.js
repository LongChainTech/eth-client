...
const requestDeleteData = function(baseUrl, input, sign) {
    return new Promise(function(resolve, reject) {
        request
            .delete(baseUrl + '/alt/cns/data')
            .send({ input: input, sign: sign })
            .end(function(err, res) {
                if (err) {
                    if (res && res.body && res.body.code === 1) {
...
const requestDeleteFile = function(baseUrl, input, sign) {
    return new Promise(function(resolve, reject) {
        request
            .delete(baseUrl + '/alt/cns/file')
            .send({ input: input, sign: sign })
            .end(function(err, res) {
                if (err) {
                    if (res && res.body && res.body.code === 1) {
...
AltExecCnsContract.prototype.deleteData = function(password, contractName, functionName, objectId, params, abi, callback) {
    try {
        const url = this.account.baseUrl;
        const cnsAddress = this.cnsAddress;
        const altParams = new AltExecCnsParams(cnsAddress, contractName, functionName, params, abi);
        altParams.setObject(objectId);
        const hash = altParams.getHashedParams();
        const input = altParams.getInput();

        this.account.sign(password, hash).then(function(sign) {
            return requestDeleteData(url, input, sign);
        }).then(function(result) {
            callback(null, result);
        }).catch(function(err) {
            callback(err);
...
AltExecCnsContract.prototype.deleteFile = function(password, contractName, functionName, objectId, params, abi, callback) {
    try {
        const url = this.account.baseUrl;
        const cnsAddress = this.cnsAddress;
        const altParams = new AltExecCnsParams(cnsAddress, contractName, functionName, params, abi);
        ...
        this.account.sign(password, hash).then(function(sign) {
            return requestDeleteFile(url, input, sign);
        }).then(function(result) {
            callback(null, result);
        }).catch(function(err) {
            callback(err);
