...
const requestPostFileinNodeJS = function(baseUrl, input, sign, data, filePath) {
    return new Promise(function(resolve, reject) {
                ...
                if (err) {
                    ...
                    reader.readAsText(res.xhr.response);
                     ...
                     try{
                        ...
                        if(err.name == 'ReferenceError' && err.message == 'File is not defined'){
                            resolve(res);
                        } else {
                            callback(err);
                        }
...
const sendFileinNodeJS = function(password, contractName, functionName, objectId, data, filePath, params, abi, callback, _this) {
    try {
        var fileSize = fs.statSync(filePath);
        if ( fileSize > MAX_FILE_SIZE) throw 'File too large';
        const dataHash = ethUtil.addHexPrefix(utils.hash(data));
        const altParams = new AltExecCnsParams(_this.cnsAddress, contractName, functionName, params, abi);
        utils.hashFileformPath(filePath).then(function(fileHash) {
            altParams.setObject(objectId, dataHash, ethUtil.addHexPrefix('0x' + fileHash));
            return Promise.resolve(altParams.getHashedParams());
        }).then(function(hash) {
            return _this.account.sign(password, hash);
        }).then(function(sign) {
            return requestPostFileinNodeJS(_this.account.baseUrl, altParams.getInput(), sign, data, filePath);
        }).then(function(result) {
            callback(null, result);
        }).catch(function(err) {
            callback(err);
...
AltExecCnsContract.prototype.sendFile = function(password, contractName, functionName, objectId, data, file, params, abi, callback) {
    try {
        ...
        utils.hashFile(file).then(function(fileHash) {
            ...
            if(err.name == 'ReferenceError' && err.message == 'File is not defined'){
                sendFileinNodeJS(password, contractName, functionName, objectId, data, file, params, abi, callback, _this);
            } else {
                callback(err);
            }
        });
